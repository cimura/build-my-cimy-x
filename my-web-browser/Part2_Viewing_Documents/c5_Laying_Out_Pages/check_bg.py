...
class HTMLParser:
	def __init__(self, body):
		...
		print(f"in HTMLParser in init type:{type(body)}")
		print(f"in HTMLParser in init body:{(body)}")
...
class BlockLayout:
	...
	def paint(self):
		...
		if isinstance(self.node, Element) and self.node.tag == "pre":
			...
			cmds.append(rect)
			print(f"DrawRect created: left={self.x}, top={self.y}, right={x2}, bottom={y2}")
		
		# インラインモードの場合のみテキストを描画
		if self.layout_mode() == "inline":
			for x, y, word, font in self.display_list:
				cmds.append(DrawText(x, y, word, font))
		print(f"BlockLayout.paint() returning {len(cmds)} commands for node: {self.node}")	
		return cmds
	...
	def layout(self):
		...
		print(f"Layout mode for {self.node}: {mode}")
		...
		if isinstance(self.node, Element) and self.node.tag == "pre":
			print(f"PRE layout completed: height={self.height}, children={len(self.children)}")
